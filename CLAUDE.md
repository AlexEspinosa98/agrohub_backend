...
- `apps.asistencia_eventos` (`/asistencia-eventos/`, admin/superadmin only) — OCR digitization of handwritten event attendance sheets. Two-step flow: `POST /scan` runs PaddleOCR over an uploaded PDF/image and returns everything it extracted as an unsaved draft; `POST /eventos` takes the human-corrected version of that same payload (plus the original file) and actually saves it — nothing is written to the DB until confirm. People are upserted by `numero_documento` into a master `PersonaAsistente` table (same pattern as `PersonaNutricional`), separate from the per-event `RegistroAsistencia` row. See `docs/asistencia-eventos.md` for the full contract and known OCR limitations — Tesseract was tried first and failed to read a single row on a real handwritten sample; PaddleOCR (CPU, no external API — the deployment target is onpremise) does markedly better but is still draft-quality, which is why the confirm step exists.
